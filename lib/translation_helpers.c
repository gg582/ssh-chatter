...
bool
translation_prepare_text (const char *message, char *sanitized,
...
bool
translation_restore_text (const char *translated, char *output,
                          size_t output_len,
...
bool
translation_strip_no_translate_prefix (const char *message, char *stripped,
                                       size_t stripped_len)
{
...
static int
get_utf8_char_display_width (const char *s, size_t max_len, size_t *bytes_read)
{
  mbstate_t ps;
...
int
get_display_width (const char *text)
{
  if (text == NULL) {
    return 0;
  }
...
static size_t
extract_ansi_sequence (const char *text, size_t max_len, char *buffer,
                       size_t buffer_len)
{
...
char **
wrap_text_to_width (const char *text, int max_width, size_t *line_count)
{
  ...
  while (*current_pos != '\0') {
    ...
    if (strlen (active_ansi_codes) > 0) {
      ...
      if (current_line_len + reset_len + ansi_len
        ...
        memcpy (current_line_buffer + current_line_len, active_ansi_codes,
                ansi_len);
    ...
    while (*current_pos != '\0') {
      ...
      if (*current_pos == '\033') {
        ansi_seq_len
            = extract_ansi_sequence (current_pos, strlen (current_pos),
                                     ansi_seq_buffer, sizeof (ansi_seq_buffer));
        if (ansi_seq_len > 0) {
          // Append ANSI sequence to current line buffer
          if (current_line_len + ansi_seq_len < sizeof (current_line_buffer)) {
            memcpy (current_line_buffer + current_line_len, ansi_seq_buffer,
                    ansi_seq_len);
          ...
          strncat (active_ansi_codes, ansi_seq_buffer,
                   sizeof (active_ansi_codes) - strlen (active_ansi_codes) - 1);
