...
static int
translator_timespec_compare (const struct timespec *a, const struct timespec *b)
{
  if (a == NULL || b == NULL) {
    return 0;
  }
...
static struct timespec
translator_timespec_add_ns (const struct timespec *base, long nanoseconds)
{
...
static struct timespec
translator_timespec_diff (const struct timespec *end,
...
static void
translator_gemini_cooldown_write_snapshot (long long remaining_ns)
{
...
static void
translator_gemini_cooldown_persist_locked (void)
{
...
static void
translator_gemini_cooldown_resolve_path (void)
{
...
static void
translator_gemini_cooldown_load (void)
{
  if (g_gemini_cooldown_file_path[0] == '\0') {
    return;
  }
...
static void
translator_rate_limit_wait (void)
{
  for (;;) {
...
static void
translator_moderation_throttle_wait (void)
{
  for (;;) {
...
static void
translator_rate_limit_penalise_until (const struct timespec *until)
{
  if (until == NULL) {
    return;
  }
...
static void
translator_rate_limit_penalize (long status)
{
  if (status != 429L) {
    return;
  }
...
static bool
translator_cancel_requested (const volatile bool *flag)
{
  return flag != NULL && *flag;
}

static int
translator_progress_abort (void *clientp, curl_off_t dltotal, curl_off_t dlnow,
...
static int
translator_progress_abort_legacy (void *clientp, double dltotal, double dlnow,
...
static void
translator_clear_gemini_backoff_locked (void)
{
...
static void
translator_clear_gemini_backoff_internal (void)
{
  pthread_mutex_lock (&g_provider_mutex);
  translator_clear_gemini_backoff_locked ();
  pthread_mutex_unlock (&g_provider_mutex);
...
void
translator_clear_gemini_backoff (void)
{
  translator_clear_gemini_backoff_internal ();
}

static void
translator_schedule_gemini_backoff_ns (long duration_ns)
{
  struct timespec now = translator_timespec_now ();
...
static long
translator_gemini_backoff_duration_until_midnight (void)
{
...
static void
translator_schedule_gemini_backoff_until_midnight (void)
{
...
static bool
translator_gemini_enabled_internal (void)
{
  struct timespec now = translator_timespec_now ();
  bool enabled = true;
...
void
translator_set_gemini_enabled (bool enabled)
{
  pthread_mutex_lock (&g_provider_mutex);
...
bool
translator_is_gemini_enabled (void)
{
  return translator_gemini_enabled_internal ();
}

bool
translator_is_gemini_manually_disabled (void)
{
  pthread_mutex_lock (&g_provider_mutex);
...
bool
translator_gemini_backoff_remaining (struct timespec *remaining)
{
  struct timespec now = translator_timespec_now ();
  bool active = false;
...
bool
translator_is_ollama_only (void)
{
...
void
translator_set_manual_chat_bbs_only (bool enabled)
{
  pthread_mutex_lock (&g_provider_mutex);
...
bool
translator_is_manual_chat_bbs_only (void)
{
  pthread_mutex_lock (&g_provider_mutex);
...
void
translator_set_manual_skip_scrollback (bool enabled)
{
  pthread_mutex_lock (&g_provider_mutex);
...
bool
translator_is_manual_skip_scrollback (void)
{
  pthread_mutex_lock (&g_provider_mutex);
...
bool
translator_should_limit_to_chat_bbs (void)
{
...
bool
translator_should_skip_scrollback_translation (void)
{
...
static size_t
translator_utf8_encode (uint32_t codepoint, char *output, size_t max_len)
{
...
static bool
translator_parse_hex4 (const char *input, uint32_t *value)
{
...
static bool
translator_decode_json_string (const char *input, char *output,
...
static void
translator_mark_quota_exhausted (void)
{
...
static void
translator_set_error (const char *fmt, ...)
{
  pthread_mutex_lock (&g_error_mutex);
  if (fmt == NULL) {
    g_last_error[0] = '\0';
...
bool
translator_last_error_was_quota (void)
{
  pthread_mutex_lock (&g_error_mutex);
...
static size_t
translator_write_callback (void *contents, size_t size, size_t nmemb,
                           void *userp)
{
...
static char *
translator_escape_string (const char *input)
{
  if (input == NULL) {
    return NULL;
  }
...
static const char *
translator_skip_whitespace (const char *cursor)
{
  if (cursor == NULL) {
    return NULL;
  }
...
static char *
translator_extract_first_text_generic (const char *response)
{
  if (response == NULL) {
    return NULL;
  }
...
static char *
translator_extract_payload_text (const char *response)
{
  if (response == NULL) {
    return NULL;
  }
...
static bool
translator_extract_plaintext_response (const char *response, char *dest,
                                       size_t dest_len)
{
...
static bool
translator_extract_json_value (const char *json, const char *key, char *dest,
                               size_t dest_len)
{
...
static char *
translator_extract_last_text_block (const char *response)
{
  if (response == NULL) {
    return NULL;
  }
...
static bool
translator_parse_moderation_decision (const char *json, bool *blocked,
...
static bool
translator_string_contains_case_insensitive (const char *haystack,
                                             const char *needle)
{
...
static char *
translator_build_gemini_url (const char *base, const char *model,
...
static char *
translator_build_ollama_url (const char *base)
{
...
static CURLcode
translator_issue_gemini_request (CURL *curl, const char *url,
...
static bool
translator_handle_payload (const char *response, char *translation,
                           size_t translation_len, char *detected_language,
                           size_t detected_len)
{
...
static CURLcode
translator_issue_json_post (CURL *curl, const char *url, const char *body,
...
static bool
translator_candidate_configure (translator_candidate_t *candidate,
                                translator_provider_t provider,
                                const char *model)
{
  if (candidate == NULL) {
                                   ...
                                   const char *model)
{
  ...
  for (size_t idx = 0U; idx < count; ++idx) {
    const char *existing_model
        = candidates[idx].model != NULL ? candidates[idx].model : "";
...
static bool
translator_add_candidate (translator_candidate_t *candidates, size_t *count,
                          size_t capacity, translator_provider_t provider,
                          const char *model)
{
...
static bool
translator_try_gemini (const translator_candidate_t *candidate,
                       ...
                       const volatile bool *cancel_flag, bool *retryable)
{
  if (retryable != NULL) {
    *retryable = false;
  }
  ...
  const char *model_name
      = candidate->model != NULL && candidate->model[0] != '\0'
            ? candidate->model
  ...
  if (stream_url != NULL) {
    ...
    if (stream_result == CURLE_ABORTED_BY_CALLBACK
      ...
      translator_set_error ("Translation canceled.");
               ...
               && stream_status < 300L && stream_buffer.data != NULL) {
...
static bool
translator_try_gemini_eliza (const translator_candidate_t *candidate,
                             const char *prompt, char *reply, size_t reply_len,
                             bool *retryable)
{
  if (retryable != NULL) {
    *retryable = false;
  ...
  int computed
      = snprintf (NULL, 0, body_format, escaped_system, escaped_prompt);
  if (computed < 0) {
    translator_set_error ("Failed to prepare eliza request payload.");
    return false;
  }
  ...
  CURLcode result = translator_issue_gemini_request (
      curl, api_url, api_key, body, false, NULL, &buffer, &status);
  if (result != CURLE_OK) {
    translator_set_error ("Failed to contact Gemini API: %s",
                          curl_easy_strerror (result));
    if (retryable != NULL) {
    ...
    message[0] = '\0';
    if (buffer.data != NULL) {
      (void)translator_extract_json_value (buffer.data, "\"message\"", message,
                                           sizeof (message));
    }
...
translator_try_gemini_moderation (const translator_candidate_t *candidate,
                                  ...
                                  size_t reason_len, bool *retryable)
{
  if (retryable != NULL) {
    *retryable = false;
  }
  ...
  char *escaped_content
      = translator_escape_string (content != NULL ? content : "");
  ...
  int body_length
      = snprintf (NULL, 0, body_format, escaped_category, escaped_content);
  if (body_length < 0) {
    translator_set_error ("Failed to prepare moderation request payload.");
    return false;
  }
...
static bool
translator_try_ollama (const translator_candidate_t *candidate,
                       const char *text, const char *target_language,
                       char *translation, size_t translation_len,
                       char *detected_language, size_t detected_len,
                       const volatile bool *cancel_flag, bool *retryable)
{
  ...
  if (translation == NULL || translation_len == 0U || text == NULL
  ...
  int body_length = snprintf (NULL, 0, body_format, model_name, escaped_prompt,
                              escaped_system);
  if (body_length < 0) {
  ...
  CURLcode result = translator_issue_json_post (
      curl, url, body, NULL, NULL, NULL, cancel_flag, &buffer, &status);
  ...
  if (result == CURLE_ABORTED_BY_CALLBACK
    ...
    message[0] = '\0';
    if (buffer.data != NULL) {
      (void)translator_extract_json_value (buffer.data, "\"error\"", message,
                                           sizeof (message));
      if (message[0] == '\0') {
        (void)translator_extract_json_value (buffer.data, "\"message\"",
                                             message, sizeof (message));
    ...
    if (!translator_extract_json_value (buffer.data, "\"response\"", content,
      ...
      (void)translator_extract_json_value (content, "\"detected_language\"",
                                           detected, sizeof (detected));
...
static bool
translator_try_ollama_eliza (const translator_candidate_t *candidate,
                             const char *prompt, char *reply, size_t reply_len,
                             bool *retryable)
{
  if (retryable != NULL) {
  ...
  int computed = snprintf (NULL, 0, body_format, model_name, escaped_prompt,
                           escaped_system);
  if (computed < 0) {
    translator_set_error ("Failed to prepare eliza request.");
    return false;
  ...
  translator_buffer_t buffer = { 0 };
  long status = 0L;
  CURLcode result = translator_issue_json_post (curl, url, body, NULL, NULL,
                                                NULL, NULL, &buffer, &status);
                                  ...
                                  size_t reason_len, bool *retryable)
{
  ...
  int prompt_length = snprintf (NULL, 0, prompt_format, label,
...
static size_t
translator_prepare_candidates (translator_candidate_t *candidates,
                               size_t capacity)
{
  ...
  if (translator_gemini_enabled_internal ()) {
    ...
    if (env_model != NULL && env_model[0] != '\0' && count < capacity) {
      (void)translator_add_candidate (candidates, &count, capacity,
                                      TRANSLATOR_PROVIDER_GEMINI, env_model);
    }
...
translator_translate_internal (const char *text, const char *target_language,
                               ...
                               const volatile bool *cancel_flag)
{
  if (text == NULL || target_language == NULL || translation == NULL
      || translation_len == 0U) {
    return false;
  }
  ...
  for (size_t idx = 0U; idx < candidate_count; ++idx) {
    ...
    case TRANSLATOR_PROVIDER_GEMINI:
      success = translator_try_gemini (
          &candidates[idx], text, target_language, translation, translation_len,
...
bool
translator_translate_with_cancel (const char *text, const char *target_language,
                                  char *translation, size_t translation_len,
                                  char *detected_language, size_t detected_len,
                                  const volatile bool *cancel_flag)
{
...
bool
translator_translate (const char *text, const char *target_language,
                      char *translation, size_t translation_len,
                      char *detected_language, size_t detected_len)
{
  return translator_translate_internal (text, target_language, translation,
...
bool
translator_eliza_respond (const char *prompt, char *reply, size_t reply_len)
{
  ...
  for (size_t idx = 0U; idx < candidate_count; ++idx) {
    ...
    switch (candidates[idx].provider) {
    case TRANSLATOR_PROVIDER_GEMINI:
      success = translator_try_gemini_eliza (&candidates[idx], prompt, reply,
                                             reply_len, &retryable);
      break;
    case TRANSLATOR_PROVIDER_OLLAMA:
      success = translator_try_ollama_eliza (&candidates[idx], prompt, reply,
                                             reply_len, &retryable);
      break;
    }
...
bool
translator_moderate_text (const char *category, const char *content,
  ...
  for (size_t idx = 0U; idx < candidate_count; ++idx) {
    ...
    case TRANSLATOR_PROVIDER_GEMINI:
      success = translator_try_gemini_moderation (
          &candidates[idx], category, content, &local_blocked, local_reason,
