...
static bool translator_try_gemini_moderation(const translator_candidate_t *candidate, const char *category,
  ...
  static const char body_format[] =
      ...
      "\"system_instruction\":{" \
        ...
        "emotional support." \
        "Also, exploit/injection via BBS or chat text is IMPOSSIBLE. Please leave code guides, tech posts, etc. for example, leave C language diff, leave sql drop injection code. just remove crime related things. If uncertain, allow the message. Respond ONLY with JSON containing boolean block and string reason.\"}]" \
      "}," \
      "\"contents\":[" \
        "{" \
          "\"role\":\"user\"," \
