...
bool ssh_chatter_backend_translate_line(const char *message, const char *target_language,
  ...
  if (message == NULL || target_language == NULL || translated == NULL || translated_len == 0U) {
    return false;
  }
  ...
  if (!translator_translate(sanitized, target_language, translated_body, sizeof(translated_body), detected_buffer,
                            sizeof(detected_buffer))) {
    return false;
  }

  char restored[SSH_CHATTER_TRANSLATION_WORKING_LEN];
  if (!translation_restore_text(translated_body, restored, sizeof(restored), placeholders, placeholder_count)) {
    return false;
  }
