...
static bool
matrix_url_is_https (const char *url)
{
  if (url == NULL) {
    return false;
  }
...
static size_t
matrix_curl_write (void *contents, size_t size, size_t nmemb, void *userp)
{
...
static void
matrix_buffer_free (matrix_buffer_t *buffer)
{
  if (buffer == NULL) {
    return;
  }
...
static void
matrix_client_disable (matrix_client_t *client, const char *message,
                       int error_code)
{
  if (client == NULL) {
    return;
...
static bool
matrix_copy_trimmed (char *dest, size_t dest_len, const char *src)
{
...
static bool
matrix_probe_homeserver (const char *homeserver)
{
...
static bool
matrix_json_escape (const char *input, char *output, size_t output_len)
{
  ...
  for (const unsigned char *cursor = (const unsigned char *)input;
    ...
    default:
      if (ch < 0x20U) {
        int written = snprintf (output + out_index, output_len - out_index,
                                "\\u%04X", ch);
...
static bool
matrix_append_codepoint (char **output, size_t *remaining,
...
static bool
matrix_json_decode_string (const char *input, char *output, size_t output_len,
                           const char **end_out)
{
...
static const char *
matrix_skip_whitespace (const char *cursor)
{
  while (cursor != NULL
         && (*cursor == ' ' || *cursor == '\n' || *cursor == '\r'
             || *cursor == '\t')) {
...
static bool
matrix_json_extract_string (const char *json, const char *key, char *output,
                            size_t output_len)
{
...
static bool
matrix_json_extract_bool (const char *json, const char *key, bool *value_out)
{
...
static const char *
matrix_attachment_label (chat_attachment_type_t type)
{
  switch (type) {
...
static bool
matrix_client_issue_request (matrix_client_t *client, const char *method,
                             const char *url, const char *body,
                             matrix_buffer_t *response, long *status_out)
{
...
static bool
matrix_client_send_payload (matrix_client_t *client, const char *payload)
{
  ...
  if (!sent) {
    ...
    snprintf (message, sizeof (message),
              "matrix send failed (HTTP %ld); matrix bridge disabled", status);
...
static bool
matrix_client_should_skip (matrix_client_t *client,
...
static bool
matrix_entry_is_bbs_notice (const chat_history_entry_t *entry)
{
...
static bool
matrix_client_build_plaintext (matrix_client_t *client,
                               const chat_history_entry_t *entry,
...
static bool
matrix_client_send_entry (matrix_client_t *client,
                          const chat_history_entry_t *entry)
{
...
static void
matrix_client_on_message (client_connection_t *connection,
                          const chat_history_entry_t *entry)
{
...
static void
matrix_client_on_detach (client_connection_t *connection)
{
  (void)connection;
}

static void
matrix_client_record_event (matrix_client_t *client, const char *event_id)
{
  ...
  for (size_t idx = 0U; idx < limit; ++idx) {
    size_t pos = (client->recent_event_head + MATRIX_MAX_EVENT_IDS - idx - 1U)
...
static bool
matrix_client_event_recent (matrix_client_t *client, const char *event_id)
{
...
static bool
matrix_client_parse_payload (const char *plaintext, matrix_payload_t *payload)
{
  ...
  if (!matrix_json_extract_string (plaintext, "\"username\"", payload->username,
  ...
  (void)matrix_json_extract_string (
      plaintext, "\"category\"", payload->category, sizeof (payload->category));

  return true;
}

static void
matrix_client_inject_message (matrix_client_t *client,
...
static void
matrix_client_handle_body (matrix_client_t *client, const char *body)
{
...
static const char *
matrix_find_matching (const char *start, char open, char close)
{
...
static void
matrix_client_process_event (matrix_client_t *client, const char *event_json,
                             size_t length)
{
...
static void
matrix_client_process_sync (matrix_client_t *client, const char *json)
{
...
static bool
matrix_client_sync (matrix_client_t *client)
{
  if (client == NULL) {
    return false;
  }
...
static void *
matrix_client_poll_thread (void *user_data)
{
...
matrix_client_t *
matrix_client_create (host_t *host, client_manager_t *manager,
                      security_layer_t *security)
{
  ...
  if (!matrix_copy_trimmed (client->room_id, sizeof (client->room_id), room_id)
      || client->room_id[0] == '\0') {
    OPENSSL_cleanse (client->access_token, sizeof (client->access_token));
    free (client);
    return NULL;
  ...
  atomic_store (&client->running, false);
  if (pthread_create (&client->thread, NULL, matrix_client_poll_thread, client)
      != 0) {
...
void
matrix_client_destroy (matrix_client_t *client)
{
  if (client == NULL) {
    return;
  }
