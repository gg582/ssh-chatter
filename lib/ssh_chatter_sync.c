...
static void *retry_connection_thread(void *arg) {
    ...
    while (sync_running) {
        if (retry_pending) {
            ...
            double elapsed = timespec_elapsed_seconds(&current_settings.last_sync_attempt, &now);
...
void ssh_chatter_sync_add_message_to_history(const chat_message_t* new_msg) {
    ...
    if (chat_history_head != NULL && strcmp(chat_history_head->message_body, new_msg->message_body) == 0 && strcmp(chat_history_head->username, new_msg->username) == 0) {
    ...
    if (node == NULL) {
        fprintf(stderr, "[SSH_SYNC] Failed to allocate memory for chat message history node.\n\n");
        pthread_mutex_unlock(&history_mutex);
        return;
    }
...
void ssh_chatter_sync_start() {
    ...
    ssh_options_set(session, SSH_OPTIONS_USER, current_settings.go_chat_username);

    int rc = ssh_connect(session);
    if (rc != SSH_OK) {
        fprintf(stderr, "[SSH_SYNC] Error connecting to %s:%d: %s\n\n",
                current_settings.go_chat_host, current_settings.go_chat_port, ssh_get_error(session));
        ssh_free(session);
        session = NULL;
    ...
    if (authenticate_ssh_session(session) != SSH_OK) {
        ...
        retry_pending = true;
        ssh_chatter_sync_save_settings();
        fprintf(stderr, "[SSH_SYNC] Authentication failed. Retrying in 1 hour.\n\n");
        // Start retry thread if not already running
        if (pthread_create(&retry_thread, NULL, retry_connection_thread, NULL) != 0) {
            fprintf(stderr, "[SSH_SYNC] Failed to create retry thread.\n\n");
        }
    ...
    sync_running = true;
    clock_gettime(CLOCK_MONOTONIC, &current_settings.last_sync_attempt); // Use monotonic clock
    ssh_chatter_sync_save_settings();
...
void ssh_chatter_sync_load_settings() {
    ...
    if (fp) {
        size_t read_bytes = fread(&current_settings, sizeof(sync_settings_t), 1, fp);
        if (read_bytes != 1) {
            fprintf(stderr, "[SSH_SYNC] Error reading settings from file, using defaults.\n\n");
...
void ssh_chatter_sync_send_message(const char* message) {
    if (!sync_running || !channel || !current_settings.sync_out_enabled) {
        fprintf(stderr, "[SSH_SYNC] Cannot send message: Sync not running, channel not open, or outgoing sync disabled.\n\n");
        return;
    }
    ...
    int rc = ssh_channel_write(channel, msg_with_newline, (uint32_t)strlen(msg_with_newline));
        ...
        if (nbytes > 0) {
            ...
            while ((next_line = strchr(line, '\n')) != NULL) {
                *next_line = '\0'; // Null-terminate the current line
                ...
                while (end > trimmed_line && (*end == ' ' || *end == '\t' || *end == '\r')) {
                    *end = '\0';
                    end--;
                }
