...
static bool
security_layer_derive_subkeys (security_layer_t *layer)
{
  if (layer == nullptr) {
    errno = EINVAL;
    return false;
...
bool
security_layer_init (security_layer_t *layer)
{
  if (layer == nullptr) {
    errno = EINVAL;
    return false;
...
void
security_layer_free (security_layer_t *layer)
{
  if (layer == nullptr) {
    return;
  }
...
bool
security_layer_encrypt_message (const security_layer_t *layer,
                                ...
                                size_t out_len)
{
  ...
  for (size_t idx = 0U; idx < SECURITY_LAYER_LEVELS; ++idx) {
    ...
    if (EVP_EncryptInit_ex (ctx, EVP_aes_256_gcm (), nullptr, nullptr, nullptr)
        ...
        || EVP_CIPHER_CTX_ctrl (ctx, EVP_CTRL_GCM_SET_IVLEN,
                                (int)sizeof (component->iv), nullptr)
               != 1
    ...
    if (working_len > 0U) {
      ...
      int tmp_len = 0;
      if (EVP_EncryptUpdate (ctx, component->ciphertext, &tmp_len, nullptr, 0)
          != 1) {
        EVP_CIPHER_CTX_free (ctx);
        errno = EIO;
        success = false;
    ...
    if (EVP_CIPHER_CTX_ctrl (ctx, EVP_CTRL_GCM_GET_TAG, SECURITY_LAYER_TAG_LEN,
                             component->tag)
        != 1) {
      EVP_CIPHER_CTX_free (ctx);
      errno = EIO;
  ...
  for (size_t idx = 0U; idx < SECURITY_LAYER_LEVELS; ++idx) {
    if (!security_layer_encode_component (
            &layers[SECURITY_LAYER_LEVELS - 1U - idx], &encoded_layers[idx])) {
      success = false;
      break;
    }
...
static bool
security_layer_decode_segment (const char *segment, unsigned char **buffer_out,
                               size_t *length_out)
{
...
bool
security_layer_decrypt_message (const security_layer_t *layer,
                                ...
                                size_t plaintext_len)
{
  ...
  for (size_t idx = 0U; idx < SECURITY_LAYER_LEVELS; ++idx) {
    ...
    if (EVP_DecryptInit_ex (ctx, EVP_aes_256_gcm (), nullptr, nullptr, nullptr)
            != 1
        || EVP_CIPHER_CTX_ctrl (ctx, EVP_CTRL_GCM_SET_IVLEN,
                                (int)SECURITY_LAYER_IV_LEN, nullptr)
               != 1
        || EVP_DecryptInit_ex (ctx, nullptr, nullptr,
                               layer->subkeys[layer_index], iv)
               != 1) {
      EVP_CIPHER_CTX_free (ctx);
      OPENSSL_cleanse (decoded, decoded_len);
      free (decoded);
    ...
    int out_len = 0;
    if (current_len > 0U) {
      if (EVP_DecryptUpdate (ctx, plaintext_layer, &out_len, current_cipher,
                             (int)current_len)
          != 1) {
    ...
    if (EVP_CIPHER_CTX_ctrl (ctx, EVP_CTRL_GCM_SET_TAG, SECURITY_LAYER_TAG_LEN,
                             (void *)tag)
        != 1) {
